...
class TimeHelper{
  ...
  Iterable<DateTime> getTimes(DateTime selectedDateTime, Timestamp startTime, Timestamp endTime, int interval) sync* {
    ...
    do {
      yield DateTime(selectedDateTime.year, selectedDateTime.month, selectedDateTime.day, hour, minute);
      minute += step.inMinutes;
      while (minute >= 60) {
        minute -= 60;
        hour++;
      }
    } while (hour < endDate.hour || (hour == endDate.hour && minute <= endDate.minute));
  }

  int differenceBetweenTwoTimeOfDays(TimeOfDay firstTime, TimeOfDay secondTime){
