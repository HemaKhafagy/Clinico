...
void startApp() async {
  ...
  runZonedGuarded(() {runProject(const MyApp());}, (error, stack) => FirebaseCrashlytics.instance.recordError(error, stack, fatal: true,),);
}
...
void runProject(MyApp myApp) async{
  ...
  if(_isLoggedIn){
   ...
   if(_accountType == AccountTypes.Doctor.name || _accountType == AccountTypes.Company.name){
     String userEmail = appData.getUserEmail(pref)!;
     await FirebaseFirestore.instance.collection(FirestoreCollections.Users.name).where('email', isEqualTo: userEmail).get().then((snapshos)  async {
       QueryDocumentSnapshot<Map<String, dynamic>> queryDocumentSnapshot  = snapshos.docs.first;
...
class _MyAppState extends State<MyApp> {
  ...
  Widget getNextHomeScreen() {
    if(!_isLoggedIn) {
      ...
      switch(type) {
        case AccountTypes.Admin: return const AdminHomeScreen();
        case AccountTypes.Doctor: return const DoctorHomeScreen();
        case AccountTypes.Company: return const CompanyHomeScreen();
        case AccountTypes.Lab: return const LabsHomeScreen();
        case AccountTypes.RaysCenter: return const RaysCenterHomeScreen();
        default: return const PatientHomeScreen();
      }
    }
  }
}